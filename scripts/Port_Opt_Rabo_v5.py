...
capital_target = capital_growth_factor * sum(capital[i] for i in range(N))
reg_capital = 0
for i in range(N):
    reg_capital += (
        capital[i]
...
maximize_R = qubo_factory.calc_maximize_ROC2(capital_target=capital_target)
...
fig, ax = plt.subplots()
ax.scatter(list(x3.values()), list(y3.values()), color="salmon")
ax.scatter(list(x2.values()), list(y2.values()), color="goldenrod")
ax.scatter(list(x1.values()), list(y1.values()), color="turquoise")
...
name = (
    "figures/Port_Opt_Rabo_v5_Cgf"
    + str(int(100 * capital_growth_factor))
    + "_points_"
